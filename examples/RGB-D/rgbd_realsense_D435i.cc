...
bool profile_changed(const vector<rs2::stream_profile> &current,
                     const vector<rs2::stream_profile> &prev);

void interpolateData(const vector<double> &vBase_times,
                     vector<rs2_vector> &vInterp_data,
                     vector<double> &vInterp_times, const rs2_vector &prev_data,
...
int main(int argc, char **argv) {
  ...
  for (rs2::sensor sensor : sensors)
    if (sensor.supports(RS2_CAMERA_INFO_NAME)) {
      ...
      // cout << "  " << index << " : " << sensor.get_info(RS2_CAMERA_INFO_NAME)
      // << endl;
  ...
  cout << " fx = " << intrinsics_cam.fx << endl;
  cout << " fy = " << intrinsics_cam.fy << endl;
  cout << " cx = " << intrinsics_cam.ppx << endl;
  cout << " cy = " << intrinsics_cam.ppy << endl;
  cout << " height = " << intrinsics_cam.height << endl;
  cout << " width = " << intrinsics_cam.width << endl;
  cout << " Coeff = " << intrinsics_cam.coeffs[0] << ", "
       << intrinsics_cam.coeffs[1] << ", " << intrinsics_cam.coeffs[2] << ", "
       << intrinsics_cam.coeffs[3] << ", " << intrinsics_cam.coeffs[4] << ", "
       << endl;
  cout << " Model = " << intrinsics_cam.model << endl;
  ...
  while (!SLAM.isShutDown()) {
    {
      ...
      chrono::steady_clock::time_point time_Start_Process =
          chrono::steady_clock::now();
#else
      chrono::monotonic_clock::time_point time_Start_Process =
...
#ifdef COMPILEDWITHC11
      chrono::steady_clock::time_point t_Start_Resize =
          chrono::steady_clock::now();
#else
      chrono::monotonic_clock::time_point t_Start_Resize =
          chrono::monotonic_clock::now();
#endif
#endif
...
#ifdef REGISTER_TIMES
#ifdef COMPILEDWITHC11
      chrono::steady_clock::time_point t_End_Resize =
          chrono::steady_clock::now();
#else
      chrono::monotonic_clock::time_point t_End_Resize =
          chrono::monotonic_clock::now();
#endif
...
#ifdef REGISTER_TIMES
#ifdef COMPILEDWITHC11
    chrono::steady_clock::time_point t_Start_Track =
        chrono::steady_clock::now();
#else
    chrono::monotonic_clock::time_point t_Start_Track =
        chrono::monotonic_clock::now();
#endif
#endif
...
#else
    chrono::monotonic_clock::time_point t_End_Track =
        chrono::monotonic_clock::now();
#endif
...
rs2_stream find_stream_to_align(const vector<rs2::stream_profile> &streams) {
  ...
  if (!depth_stream_found)
    throw runtime_error("No Depth stream available");

  if (align_to == RS2_STREAM_ANY)
    throw runtime_error("No stream found to align with Depth");
...
bool profile_changed(const vector<rs2::stream_profile> &current,
  ...
  for (auto &&sp : prev) {
    ...
    auto itr = find_if(begin(current), end(current),
                       [&sp](const rs2::stream_profile &current_sp) {
                         return sp.unique_id() == current_sp.unique_id();
                       });
    if (itr == end(current)) // If it previous stream wasn't found in current
    {
      return true;
    }
  }
