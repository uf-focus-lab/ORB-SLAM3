...
int main(int argc, char **argv) {
...
#ifdef REGISTER_TIMES
      t_track = t_resize +
                chrono::duration_cast<chrono::duration<double, milli>>(t2 - t1)
                    .count();
  ...
  chrono::system_clock::time_point scNow = chrono::system_clock::now();
  time_t now = chrono::system_clock::to_time_t(scNow);
  stringstream ss;
  ss << now;
