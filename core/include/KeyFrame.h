...
public:
  ...
  const vector<cv::KeyPoint> mvKeys;
  const vector<cv::KeyPoint> mvKeysUn;
  const vector<float> mvuRight; // negative value for monocular points
  const vector<float> mvDepth;  // negative value for monocular points
  ...
  vector<KeyFrame *> mvpLoopCandKFs;
  vector<KeyFrame *> mvpMergeCandKFs;
...
protected:
  ...
  vector<long long int> mvBackupMapPointsId;
  ...
  // For save relation without pointer, this is necessary for save/load function
  map<long unsigned int, int> mBackupConnectedKeyFrameIdWeights;
  ...
  set<KeyFrame *> mspMergeEdges;
  // For save relation without pointer, this is necessary for save/load function
  long long int mBackupParentId;
  vector<long unsigned int> mvBackupChildrensId;
  vector<long unsigned int> mvBackupLoopEdgesId;
  vector<long unsigned int> mvBackupMergeEdgesId;
  ...
  // Mutex
  mutex mMutexPose; // for pose, velocity and biases
...
public:
  ...
  const vector<cv::KeyPoint> mvKeysRight;

  const int NLeft, NRight;

  vector<vector<vector<size_t>>> mGridRight;
