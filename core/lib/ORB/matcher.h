...
public:
  ...
  int SearchByProjection(KeyFrame *pKF, Sophus::Sim3<float> &Scw,
                         const vector<MapPoint *> &vpPoints,
                         const vector<KeyFrame *> &vpPointsKFs,
                         vector<MapPoint *> &vpMatched,
                         vector<KeyFrame *> &vpMatchedKF, int th,
                         float ratioHamming = 1.0);
  ...
  int SearchForInitialization(Frame &F1, Frame &F2,
                              ...
                              vector<int> &vnMatches12, int windowSize = 10);
  ...
  // SearchBySim3(KeyFrame* pKF1, KeyFrame* pKF2, vector<MapPoint *>
  // &vpMatches12, const float &s12, const cv::Mat &R12, const cv::Mat &t12,
  // const float th);
  int SearchBySim3(KeyFrame *pKF1, KeyFrame *pKF2,
                   vector<MapPoint *> &vpMatches12, const Sophus::Sim3f &S12,
                   const float th);
...
protected:
  ...
  void ComputeThreeMaxima(vector<int> *histo, const int L, int &ind1, int &ind2,
                          int &ind3);
