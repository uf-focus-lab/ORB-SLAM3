...
void LocalMapping::Run() {
  ...
  while (1) {
    ...
    if (CheckNewKeyFrames() && !mbBadImu) {
      ...
      chrono::steady_clock::time_point time_StartProcessKF =
          chrono::steady_clock::now();
#endif
      // BoW conversion and insertion in Map
      ProcessNewKeyFrame();
#ifdef REGISTER_TIMES
      chrono::steady_clock::time_point time_EndProcessKF =
          chrono::steady_clock::now();
      ...
      MapPointCulling();
#ifdef REGISTER_TIMES
      chrono::steady_clock::time_point time_EndMPCulling =
          chrono::steady_clock::now();
...
#ifdef REGISTER_TIMES
      chrono::steady_clock::time_point time_EndMPCreation =
          chrono::steady_clock::now();
          ...
          if (mbInertial && mpCurrentKeyFrame->GetMap()->isImuInitialized()) {
            ...
            if (!mpCurrentKeyFrame->GetMap()->GetIniertialBA2()) {
              if ((mTinit < 10.f) && (dist < 0.02)) {
                cerr << "Not enough motion for initializing. Reseting..."
                     << endl;
...
#ifdef REGISTER_TIMES
        chrono::steady_clock::time_point time_EndLBA =
            chrono::steady_clock::now();

        if (b_doneLBA) {
          timeLBA_ms = chrono::duration_cast<chrono::duration<double, milli>>(
...
#ifdef REGISTER_TIMES
        chrono::steady_clock::time_point time_EndKFCulling =
            chrono::steady_clock::now();
...
#ifdef REGISTER_TIMES
      chrono::steady_clock::time_point time_EndLocalMap =
          chrono::steady_clock::now();
  ...
  if (mbInertial) {
    ...
    while ((vpNeighKFs.size() <= nn) && (pKF->mPrevKF) && (count++ < nn)) {
      vector<KeyFrame *>::iterator it =
          find(vpNeighKFs.begin(), vpNeighKFs.end(), pKF->mPrevKF);
...
void LocalMapping::Release() {
  ...
  cerr << "Local Mapping RELEASE" << endl;
}
...
void LocalMapping::RequestReset() {
  {
    unique_lock<mutex> lock(mMutexReset);
    cerr << "LM: Map reset recieved" << endl;
    mbResetRequested = true;
  }
  cerr << "LM: Map reset, waiting..." << endl;
  ...
  cerr << "LM: Map reset, Done!!!" << endl;
}

void LocalMapping::RequestResetActiveMap(Map *pMap) {
  {
    unique_lock<mutex> lock(mMutexReset);
    cerr << "LM: Active map reset recieved" << endl;
    mbResetRequestedActiveMap = true;
    mpMapToReset = pMap;
  }
  cerr << "LM: Active map reset, waiting..." << endl;
  ...
  cerr << "LM: Active map reset, Done!!!" << endl;
}
...
void LocalMapping::ResetIfRequested() {
  bool executed_reset = false;
  {
    ...
    if (mbResetRequested) {
      ...
      cerr << "LM: Reseting Atlas in Local Mapping..." << endl;
      mlNewKeyFrames.clear();
      ...
      cerr << "LM: End reseting Local Mapping..." << endl;
    }

    if (mbResetRequestedActiveMap) {
      executed_reset = true;
      cerr << "LM: Reseting current map in Local Mapping..." << endl;
      mlNewKeyFrames.clear();
      mlpRecentAddedMapPoints.clear();
  ...
  if (executed_reset)
    cerr << "LM: Reset free the mutex" << endl;
}
...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA) {
  ...
  while (!lpKFtoCheck.empty()) {
    ...
    for (set<KeyFrame *>::const_iterator sit = sChilds.begin();
      ...
      if (pChild->mnBAGlobalForKF != GBAid) {
        ...
        if (pChild->isVelocitySet()) {
          ...
          Verbose::Log("Child velocity empty!! ", Verbose::VERBOSITY_NORMAL);
        }
    ...
    if (pKF->bImu) {
      ...
      cerr << "KF " << pKF->mnId << " not set to inertial!! \n";
    }
