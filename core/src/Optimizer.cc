...
void Optimizer::BundleAdjustment(const vector<KeyFrame *> &vpKFs,
  ...
  auto solver = g2o::OptimizationAlgorithmLevenberg(std::move(solver_ptr));
...
void Optimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg,
  ...
  // cout << "build optimization graph" << endl;
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
      ...
      if (!pKFi->mpImuPreintegrated)
        cout << "Not preintegrated measurement" << endl;
...
void Optimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg,
  ...
  auto solver_ptr =
      std::make_unique<g2o::BlockSolverX>(std::move(linearSolver));
  auto solver = g2o::OptimizationAlgorithmGaussNewton(std::move(solver_ptr));
  optimizer.setAlgorithm(&solver);
