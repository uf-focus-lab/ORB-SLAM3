...
void Optimizer::GlobalBundleAdjustment(Map *pMap, int nIterations,
                                       bool *pbStopFlag,
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
      ...
      if (pKFi->bImu && pKFi->mPrevKF->bImu) {
        ...
        cerr << pKFi->mnId << " or " << pKFi->mPrevKF->mnId << " no imu"
             << endl;
    }
  }
...
void Optimizer::OptimizeEssentialGraph(KeyFrame *pCurKF,
  ...
  for (MapPoint *pMPi : vpNonCorrectedMPs) {
    ...
    if (vpBadPose[pRefKF->mnId]) {
      ...
      cerr << "ERROR: MapPoint has a reference KF from another map" << endl;
    }
  }
}
...
void Optimizer::LocalInertialBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap,
  ...
  const int Nd = min((int)pCurrentMap->KeyFramesInMap() - 2, maxOpt);
  ...
  auto solver_ptr =
      std::make_unique<g2o::BlockSolverX>(std::move(linearSolver));
  auto solver = new g2o::OptimizationAlgorithmLevenberg(std::move(solver_ptr));
  // Use smaller lambda to avoid iterating for finding optimal lambda
  solver->setUserLambdaInit(bLarge ? 1e-2 : 1e0);
  optimizer.setAlgorithm(solver);
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (!pKFi->mPrevKF) {
      cerr << "NOT INERTIAL LINK TO PREVIOUS FRAME!!!!" << endl;
      continue;
    }
    if (pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated) {
      ...
      cerr << "ERROR building inertial edge" << endl;
  }
  ...
  // cerr << "Total map points: " << lLocalMapPoints.size() << endl;
  ...
  {
    cerr << "FAIL LOCAL-INERTIAL BA!!!!" << endl;
    return;
  }
...
void Optimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg,
  ...
  // cerr << "build optimization graph" << endl;
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
      ...
      if (!pKFi->mpImuPreintegrated)
        cerr << "Not preintegrated measurement" << endl;
  ...
  set<g2o::HyperGraph::Edge *> setEdges = optimizer.edges();
...
void Optimizer::MergeInertialBA(KeyFrame *pCurrKF, KeyFrame *pMergeKF,
  ...
  vector<pair<MapPoint *, int>> pairs;
  ...
  vector<EdgeAccRW *> vear(N, (EdgeAccRW *)NULL);
  for (int i = 0; i < N; i++) {
    // cerr << "inserting inertial edge " << i << endl;
    KeyFrame *pKFi = vpOptimizableKFs[i];
