...
Atlas::~Atlas() {
  for (set<Map *>::iterator it = mspMaps.begin(), end = mspMaps.end();
       it != end;) {
    Map *pMi = *it;
...
vector<GeometricCamera *> Atlas::GetAllCameras() { return mvpCameras; }
...
void Atlas::clearAtlas() {
  unique_lock<mutex> lock(mMutexAtlas);
  /*for(set<Map*>::iterator it=mspMaps.begin(), send=mspMaps.end(); it!=send;
  it++)
  {
      (*it)->clear();
      delete *it;
